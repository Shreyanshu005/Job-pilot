    ...
    const fetchJob = async () => {
        try {
            ...
            setFormData({
                ...
                salaryMin: job.salary?.min || '',
                salaryMax: job.salary?.max || '',
                salaryType: job.salary?.type || 'Yearly',
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="text-sm text-gray-400 mt-3">Loading job...</p>
            </div>
        );
    }
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            <div className="flex items-center justify-between mb-8">
                ...
                <motion.button
                    type="button"
                    onClick={() => document.getElementById('edit-job-form').requestSubmit()}
            ...
            <form id="edit-job-form" onSubmit={handleSubmit} className="w-full">
